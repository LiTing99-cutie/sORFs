...
for i in $(awk -F, '{print $2}' $metadata | tail -n +2 | sort | uniq); do
    ...
    ## 生成workflow文件
    workflow_path=/rd1/user/lit/project/sORFs/config
    if [ "$i" == "Null" ]; then
        workflow=$workflow_path/workflow_LFQ_MBR_null.workflow
    elif [ "$i" == "Trypsin" ]; then
        workflow=$workflow_path/workflow_LFQ_MBR_trypsin.workflow
    elif [ "$i" == "Trypsin_LysC" ]; then
        workflow=$workflow_path/workflow_LFQ_MBR_trypsin_lysc.workflow
    elif [ "$i" == "Trypsin_LysN" ]; then
        workflow=$workflow_path/workflow_LFQ_MBR_trypsin_lysn.workflow
    elif [ "$i" == "Trypsin_Chymotrypsin" ]; then
        workflow=$workflow_path/workflow_LFQ_MBR_trypsin_chymotrypsin.workflow
    else
