...
################################################
#!/bin/sh 
# 【20250923新增】将RNA-SEQ大于0的基因的注释gtf append到ISO-SEQ的gtf后面
anno_gtf=/home/user/data2/lit/project/ZNF271/data/annotation/gencode.v41.annotation.gtf
gene_expr=/home/user/data3/lit/project/sORFs/06-RNA-seq/02-output-20250621/expr/rpkm_N_C_A_expressed.txt

awk -F'\t' '
  NR==FNR{
    # 第一个文件：找出“Gene_id”列索引并收集ID
    if(FNR==1){
      for(i=1;i<=NF;i++) if($i=="Geneid"){col=i; break}
      next
    }
    if(col>0 && $(col)!=""){ids[$(col)]=1}
    next
  }
  # 第二个文件：过滤GTF
  /^#/ {print; next}
...
' "$gene_expr" "$anno_gtf" > ../processed/filtered.by_gene_id.gtf

custom_gtf=/home/user/data3/lit/project/sORFs/08-Iso-seq-20250717/results/custom.gtf.with_orf.gtf
cat $custom_gtf ../processed/filtered.by_gene_id.gtf > ../processed/custom.gtf.with_orf.add.anno.filtered.by_gene_id.gtf
gtf=../processed/custom.gtf.with_orf.add.anno.filtered.by_gene_id.gtf

gtfToGenePred -geneNameAsName2 -genePredExt $gtf ../processed/custom.gtf.with_orf.add.anno.filtered.by_gene_id.gpe
