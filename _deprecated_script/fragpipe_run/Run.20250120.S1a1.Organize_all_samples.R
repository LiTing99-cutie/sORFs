...
res_path <- "/rd1/user/lit/project/sORFs/output/MS/Fragpipe_output/2025_01_20_default_ribo_database_formal_data_mouse"
output_path <- "/rd1/user/lit/project/sORFs/output/MS/stat/2025_01_20_default_ribo_database_formal_data_mouse"
# 输出文件夹所在的路径
# res_path <- args[1]
# output_path <- args[2]
sample_ms2_count_path <- "./output/MS/stat/sample_ms2_count_formal_hm.txt"
...
get_sample_ms_stat_from_path <- function(PATH){
  ...
  df <- data.frame(
    ...
    PSM_N=sapply(psm_files, function(x){read_file(x) %>% nrow()})
  )
  # 原始的行名为文件名，去除
  rownames(df) <- NULL
  return(df)
...
## 导出
fread(sample_ms2_count_path) -> sample_ms2_count
merge(l$sample_sep_ms_sta,sample_ms2_count) %>% mutate(ID_rate=PSM_N/ms2_count) -> sample_sep_ms_sta
