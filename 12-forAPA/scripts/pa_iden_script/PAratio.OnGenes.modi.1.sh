...
perlScript='/rd1/user/lit/project/271/PA/src/Macaque-transcriptome/scripts/'
perlMyself='/rd1/user/lit/project/271/PA/src/Macaque-transcriptome/scripts/'
#1. Assign reads to genes
perl $perlScript/readsAssigner.pl -g $geneStructure -s $reads >$outDir/assign.Gene.bed12+
#1.1 Add to fill internal priming reads
~/rd1/anaconda3/bin/python /rd1/user/lit/project/271/internal_priming_rm/bin/ip.py -i $outDir/assign.Gene.bed12+ -o $outDir/noIP.Gene.bed12+ -t 7 -g $fa &> fil_ip.log
#2. Classify reads into unique, ambiguous or novel based on assigned gene number
perl $perlMyself/readGroup_YM.pl -g $geneStructure $outDir/noIP.Gene.bed12+ >$outDir/unambi.bed12+ 2>$outDir/ambiguous.bed12+
