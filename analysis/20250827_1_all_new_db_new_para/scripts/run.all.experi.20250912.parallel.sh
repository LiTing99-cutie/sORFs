...
run_one() {
  ...
  bash -c '
    ...
    log="logs/'"$sample"'.log"
    {
      ...
      pfind_closed=$(ls ../processed/pFind_res_20250829/closed/*"'"$sample"'"*.spectra 2>/dev/null | head -n1 || true)
      pfind_open=$(  ls ../processed/pFind_res_20250829/open/*"'"$sample"'"*.spectra   2>/dev/null | head -n1 || true)
