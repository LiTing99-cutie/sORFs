...
closed_dir=../processed/pFind_res_20251026/closed
open_dir=../processed/pFind_res_20251026/open
...
# closed
find "$closed_dir" -type f -name 'pFind-Filtered.spectra' \
  -exec sh -c 'head -n1 -- "$1"' _ {} \; -quit \
  > "$closed_dir/closed.res.header.txt"
# open
find "$open_dir"   -type f -name 'pFind-Filtered.spectra' \
  -exec sh -c 'head -n1 -- "$1"' _ {} \; -quit \
  > "$open_dir/open.res.header.txt"
for sample in $(cut -f 1 $meta_file -d ','|sed 's/-/_/g');do
    ...
    grep -R -h --include='pFind-Filtered.spectra' -F -- "$sample" "$closed_dir" > "$closed_dir/$sample.spectra.tmp"
    ...
    grep -R -h --include='pFind-Filtered.spectra' -F -- "$sample" "$open_dir" > "$open_dir/$sample.spectra.tmp"
