...
lib_text()
lib_plot()
setwd("/home/user/data3/lit/project/sORFs/01-ribo-seq/analysis/Test-20241221-revision/")
...
fread_c(file_1) -> df_1
fread_c(file_5) -> df_5
df_5$Sample <- str_extract(df_5$Sample,"110.*gz") 
merge(df_1,df_5) -> tmp
...
total_reads$Sample <- total_reads$file %>% str_extract(.,"110.*gz")
# 双端数据只需要一端的reads就可以
total_reads[c(2,4),] -> total_reads
total_reads$Sample <- c("110N_trimmed.fastq.gz","110T_trimmed.fastq.gz")
...
p_sites_number$Sample <- p_sites_number$file %>% str_extract(.,"110.*?gz")
