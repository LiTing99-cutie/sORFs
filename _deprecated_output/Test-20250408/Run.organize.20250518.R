...
lib_text()
lib_plot()
setwd("/home/user/data3/lit/project/sORFs/01-ribo-seq/analysis/Test-20250408/")
...
##### 三碱基周期性 #####
df <- fread_c(file_4)
df %>% .[seq(2,nrow(df),2),] -> df
df %>% dplyr::rename(Sample=V1) -> df
...
fread_c(file_1) -> df_1
fread_c(file_5) -> df_5
df_5$Sample <- str_extract(df_5$Sample,Pattern) 
merge(df_1,df_5) -> tmp
tmp %>% merge(sample_periodicity[,c(1,5,6)]) -> tmp_1
# 需要修改输出路径
...
# 需要看情况修改
total_reads$Sample <- total_reads$file %>% str_extract(.,"[A-Za-z]\\d{2}_\\d{4}") %>% paste0(".raw")
# # 双端数据只需要一端的reads就可以 【需要看情况修改】
# total_reads[c(2,4),] -> total_reads
# total_reads$Sample <- c()
...
p_sites_number$Sample <- p_sites_number$file %>% str_extract(.,Pattern)
merge(p_sites_number[,c(2,3)],tmp_2) -> tmp_3
tmp_3$P_site_rate <- tmp_3$P_site_number/tmp_3$Raw_reads_number

colnames(tmp_3) -> cols
colnames_reorder <- c(cols[1],cols[3],cols[6:8],cols[c(5,4)],cols[9],cols[10:11],cols[c(2,12)])
tmp_4 <- tmp_3[,colnames_reorder]
fwrite_c(tmp_4,"./01-output/stat/stat.all.txt")
