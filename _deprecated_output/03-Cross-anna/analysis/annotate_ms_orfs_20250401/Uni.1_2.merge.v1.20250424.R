...
args <- commandArgs(T)
if(is.na(args[1])){
  sep_path = "/home/user/data3/lit/project/sORFs/03-Cross-anna/analysis/annotate_ms_orfs_20250401/output/S7/all_putative_sep_undetected_sampled.txt"
  output_file = "/home/user/data3/lit/project/sORFs/03-Cross-anna/analysis/annotate_ms_orfs_20250401/output/S7/all_putative_sep_undetected_sampled.tab_info.txt"
  update_id_or_not=FALSE
  # 每个微蛋白被多少独特的psm以及肽段支持
  ms_info_path = NA
}else{
  sep_path <- args[1]
  output_file <- args[2]
  update_id_or_not<- args[3]
  ms_info_path <- args[4]
}
...
if(update_id_or_not){
  update_orf_id_trans(all_orfs) -> all_orfs
}
# 提取染色体、起始位置、终止位置、链方向等信息
matches <- str_match(all_orfs$ORF_id_trans, "(ENS\\w+\\.\\d+[a-zA-Z_]*)([+-])(chr\\w+):(\\d+)-(\\d+)")
