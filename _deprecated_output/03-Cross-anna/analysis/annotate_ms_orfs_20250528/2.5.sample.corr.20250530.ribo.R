...
rpkm_file <- "/home/user/data3/lit/project/sORFs/03-Cross-anna/analysis/annotate_ms_orfs_20250528/output/S2/fc_output_ribo_seq/ribo_seq_combined_RPKM.txt"
fread_c(rpkm_file) -> rpkm
metadata_file <- "/home/user/data3/lit/project/sORFs/03-Cross-anna/analysis/annotate_ms_orfs_20250528/output/S2/sample_info_ribo_seq.txt"
fread_c(metadata_file) -> metadata
output_path <- "./output/S2/plot/ribo"
...
# 处理样本名
colnames(rpkm) <- sub("_.*Seq", "", colnames(rpkm))
##### 预处理 #####
rpkm %>% tibble::column_to_rownames("GeneID") %>% as.matrix() -> rpkm_matrix
...
annotation_col <- metadata_1[, c("Source", "Developmental_stage_1", "Ananomical_region","Developmental_stage")]
...
pdf(file = o("expr_heatmap.pdf"),height = 10,width = 25)
expr_heatmap
dev.off()
###### 测试不同的参数组合###### 
# 定义绘图函数
...
pdf(file = o("final_plot.pdf"),height = 30,width = 60)
final_plot
dev.off()
...
ggsave(corr_plot,filename = o("corr_plot.pdf"),height = 30,width = 30)
...
ggsave(pca_plot,filename = o("pca_plot.pdf"),height = 8,width = 8)
