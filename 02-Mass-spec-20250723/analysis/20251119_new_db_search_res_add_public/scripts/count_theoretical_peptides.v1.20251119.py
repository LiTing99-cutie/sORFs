...
def detect_rule(sample: str):
    ...
    # 无法识别，默认使用 trypsin 规则
    # print(f"警告: 无法识别 Sample='{sample}' 的酶切规则，使用默认 trypsin 规则")
    return {"cterm": set("KR"), "nterm": set(), "proline_block_cterm": True}
...
def main():
    ...
    theo_counts = []
    unrecognized_samples = set()
    
    for sample, seq in zip(df["Sample"].astype(str), df["ORF_seq"].astype(str)):
        rule = detect_rule(sample)
        # 记录无法识别的样本（只记录一次）
        if sample not in unrecognized_samples and not any(sample.endswith(suffix) for suffix in 
            ["T_T", "LC_T", "T_AC", "LN_T", "T_C", "T_GC", "T_AN"]):
            unrecognized_samples.add(sample)
    ...
    #     print(f"[警告] 以下样本无法识别，已使用默认 trypsin 规则：")
    #     for s in sorted(unrecognized_samples):
    #         print(f"  - {s}")
