...
# -*- coding: utf-8 -*-
"""
annotate_snp_effect.py (Fixed Version with Debug)
一体化流程：
...
4) 对重叠记录进行跨剪接的密码子定位与功能注释（synonymous / missense / nonsense / stop_loss / stop_retained / start_gained / unknown）。
...
def parse_orf_segments(orf_gtf: str, debug: bool = False) -> Dict[str, Dict]:
    ...
    for oid, info in raw.items():
        ...
        if strand == '+':
            ...
            ordered = sorted(merged, key=lambda x: (-x[0], -x[1]))  # 修改：按起始坐标降序
...
def annotate_row(row, orf_map, ref: pysam.FastaFile, debug_negative: bool = False) -> Dict[str,str]:
    ...
    if debug:
        print(f"  密码子起始索引: {codon_start_idx}, 密码子内位置: {codon_pos}")
    ...
    if ref_aa == 'X' or alt_aa == 'X':
        ...
        if ref_aa == '*' and alt_aa != '*':
            ...
            mtype = 'stop_retained'
    elif alt_aa == 'M' and ref_aa != 'M':  # 新的起始密码子
        mtype = 'start_gained'
    else:
        mtype = 'missense'
    ...
    out.update({
        ...
        'alt_aa': alt_aa,
        'mutation_type': mtype,
        'aa_change': f'{ref_aa}{(codon_start_idx//3)+1}{alt_aa}',
        'computed_codon_number': (codon_start_idx//3)+1,
