...
#' 绘制带柱状图的累积蛋白/肽段曲线
#' @param df 输入数据框（必须包含Sample和Protein/Peptide列）
#' @param sample_order 样本顺序向量
#' @param target_col 目标列名（"Protein"或"Peptide"）
#' @param show_bar 是否显示柱状图（TRUE/FALSE）
#' @param bar_color 柱状图颜色（默认hcl.colors(3,"Dark 3")[3]）
#' @param line_color 折线颜色（默认hcl.colors(3,"Dark 3")[1]）
#' @return 包含绘图对象和数据的列表
