...
# ---- 数据结构 ----
Exon   = namedtuple("Exon",   "start end")           # 区间（1-based, 闭区间）
CdsSeg = namedtuple("CdsSeg", "start end frame")     # 参考CDS片段，带frame(0/1/2或.)
Feature = namedtuple("Feature", "chrom source ftype start end score strand frame attrs")

# ---- GTF属性解析/序列化 ----
def parse_attrs(attr_field: str) -> dict:
    out = {}
    if not attr_field or attr_field == ".": return out
...
# ---- 读取参考GTF：收集exon/CDS(含frame)/codon；构建gene->tx索引 ----
def read_gtf_build_reference(ref_gtf_path):
    ...
    with open(ref_gtf_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            chrom, source, ftype, start, end, score, strand, frame_col, attr = cols
            ...
            if ftype in ("exon", "CDS", "start_codon", "stop_codon") and tid:
                ...
                if ftype == "exon":
                    ...
                    t["cds"].append(CdsSeg(start, end, frame_col))
    ...
    for t in tx.values():
        ...
        t["cds"].sort(key=lambda c: (c.start, c.end))
...
# ---- 候选参考转录本选择（FSM/ISM只比associated_transcript，其它比associated_gene内所有转录本） ----
...
# ---- 从CDS推导codon（仅在参考缺codon时；用Exon列表） ----
def derive_codons_from_cds(cds_exons, strand):
    if not cds_exons: return [], []
    cds_union = merge_intervals(cds_exons)
...
# ---- 生成统一UTR（不分5'/3'），用于输出末尾的UTR行 ----
def make_utr_generic_on_pb_exons(pb_exons, ref_cds_exons):
    if not ref_cds_exons: return []
    cds_union = merge_intervals(ref_cds_exons)
    utr = []
    for ex in pb_exons:
...
def main(ref_gtf, pb_gtf, classify_txt):
    ...
    # 以 associated_gene 聚集；记录 gene 跨度
    ...
    for pb_id, pb_d in pb_tx.items():
        ...
        # 本转录本输出（顺序：transcript → 逐外显子(exon→CDS→start→stop) → UTR）
        rows = []
        ...
        cds_blocks = []          # CdsSeg 列表（含frame）
        cds_exons  = []          # 仅start/end的Exon列表，用于union/UTR/codon推导
        starts = []; stops = []

        if ref_hit:
            rd = ref_tx[ref_hit]
            cds_blocks = list(rd["cds"])
            cds_exons  = [Exon(c.start, c.end) for c in cds_blocks]
            if not rd["start_codons"] or not rd["stop_codons"]:
                s, p = derive_codons_from_cds(cds_exons, strand)
        ...
        for ex in exon_iter:
            ...
            if ref_hit:
                ...
                for c in cds_here:
                    frame_out = c.frame if c.frame in ("0","1","2") else "."
                    rows.append(Feature(chrom, "PacBio", "CDS", c.start, c.end, ".", strand, frame_out,
                                        {"gene_id": assoc_gene, "transcript_id": pb_id,
                                         "source_transcript": ref_hit,
                                         "source_gene": ref_tx[ref_hit]["gene_id"],
                                         "orf_status": "transferred"}))
                # start_codon 在前，stop_codon 在后
                for s in starts:
        ...
        # UTR（统一的UTR，放在转录本末尾）
        if ref_hit:
            utrs = make_utr_generic_on_pb_exons(exons, cds_exons)
            for u in utrs:
    ...
    # 写出：gene行 + 该gene下所有PB转录本（各自内部已按要求排好）
    ...
    with open(out_path, "w", encoding="utf-8") as out:
        ...
        for gid in sorted(bucket.keys()):
            ...
            # 组内转录本排序：chrom,strand,tx_start,tx_end,transcript_id
            bucket[gid].sort(key=lambda rows: (rows[0].chrom, rows[0].strand,
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Transfer ORF annotations to PB GTF (inherit CDS frame, strand-aware exon ordering, group by associated_gene, add gene feature).")
