...
# 对于每一个样本结果路径进行循环
# find $output_path -name output
sample_output_path=/home/user/data3/lit/project/sORFs/01-ribo-seq/mouse_brain_output_20241011/2015_Science_ChoJ_hippocampus/SRR2163083/output
sample=$(echo $sample_output_path | awk -F'/' '{print $(NF-1)}')

##### PRICE #####
# mkdir -p $sample_output_path/Ribo-ORFs/PRICE
mkdir -p $sample_output_path/Ribo-ORFs-clean/PRICE
# cd $sample_output_path/Ribo-ORFs/PRICE
cd $sample_output_path/Ribo-ORFs-clean/PRICE
...
samtools index -@ 40 *_Aligned.sortedByCoord.out.bam
...
mkdir -p $sample_output_path/Ribo-ORFs-clean/RibORF
...
cd $sample_output_path/Ribo-ORFs-clean/RibORF
