...
# 功能：高效提取指定长度reads（支持默认24-36nt范围）
# 用法：./extract_reads.sh <input.bam> [lengths.txt] <output.bam>

# 检查输入参数
if [ $# -lt 2 ] || [ $# -gt 3 ]; then
    echo "Usage: $0 <input.bam> [lengths.txt] <output.bam>"
    echo "Note: If lengths.txt is omitted, will extract 24-36nt reads by default"
    exit 1
fi
...
if [ "$DEFAULT_RANGE" = 1 ]; then
    # 默认24-36nt范围
    LENGTH_COND='length($10)>=24 && length($10)<=36'
    echo "Using DEFAULT range 24-36nt"
else
    # 从文件读取目标长度
