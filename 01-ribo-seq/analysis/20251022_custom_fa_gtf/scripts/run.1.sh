...
mergd_bam_out_path=../processed/merged_bam
...
samtools merge -@ 30 -f -o $out \
  $output_path/alignment/*Aligned.toTranscriptome.out.bam
out=$mergd_bam_out_path/Aligned.sortedByCoord.out.bam
samtools merge -@ 30 -f -o $out \
  $output_path/alignment/*Aligned.sortedByCoord.out.bam

# 4、鉴定ORF
echo -e "***Call ORFs at $(date '+%Y-%m-%d %H:%M:%S')"
bash call-orfs/call.orfs.20251010.sh $(realpath ../processed/orfs) \
