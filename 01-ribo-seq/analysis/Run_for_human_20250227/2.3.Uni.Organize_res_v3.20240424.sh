# modify according to the /home/user/data3/lit/project/sORFs/01-ribo-seq/S3.0.Uni.Organize_res_v2.sh
# 将每个软件输出的结果统一
...
cd $work_path
sub_dir=$6
mkdir -p $sub_dir/RibORF $sub_dir/PRICE $sub_dir/RiboCode
cd $sub_dir
# 20250331修改
...
organize_res_script_path=/home/user/data3/lit/project/sORFs/01-ribo-seq/analysis/Run_for_human_20250227/organize_res_script
translate_script=$project_path/S3.0c.Uni.translate_gtf.v2.20250325.sh
...
# 20250331修改
riborf_script=$organize_res_script_path/S3.0a.Uni.Format_RibORF_v2.R
ribocode_script=$organize_res_script_path/S3.0f.Uni.Filter_RiboCode.R
...
python $price_script $output_file $genePred $pvalue_cutoff $only_cano_Scodon $only_cano_type nonCano.formatted.gpe
## 根据gpe得到fasta文件
...
## 过滤结果
echo $ribocode_script $output_1 $pvalue_cutoff $only_cano_Scodon $only_cano_type
Rscript $ribocode_script $output_1 $pvalue_cutoff $only_cano_Scodon $only_cano_type nonCano.sorf.txt
## 得到fasta文件
...
Rscript $riborf_script $output_1 $output_2 $stat_cutoff $riborf_cutoff $only_cano_Scodon $only_cano_type nonCano.sorf.formatted.gpe
