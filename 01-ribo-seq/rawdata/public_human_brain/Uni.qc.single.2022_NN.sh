...
if [ $rm_adpt = "yes" ]; then
	...
	# 增加了在5'端去除3个碱基的选项
	time trim_galore --clip_R1 3 --adapter $adapter -j 8 -q 20 --length 20 $raw_fastq --gzip -o output/trimmed_fastq &> log/trim_galore.log
	fi
fi
## 再次fastqc
if [ $trimmed_fastqc = "yes" ]; then
	echo -e "Trimmed reads fastqc at $(date '+%Y-%m-%d %H:%M:%S')"
	[ -d output/trimmed_fastqc ] || mkdir output/trimmed_fastqc
	trimmed_fastq=$(ls output/trimmed_fastq/*gz)
	fastqc -o output/trimmed_fastqc -t 10 $trimmed_fastq &> log/trimmed_fastqc.log
fi